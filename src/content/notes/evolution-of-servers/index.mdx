...
date: 2025-03-31
tags: ['v0.1.0', 'servers', 'serverless', 'cloudflare', 'edge', 'fluid-compute']
image: './banner.webp'
authors: ['pronsh']
---
...
Built by the language of the web, for the web (and on edge like a reverse proxy) it understands webstandards and takes advatnage of js v8 so makes sense (especially since it was born really early on in the serverless movement).
...
I believe deno deploy also does this but unsure!
Google Cloud Run also had this its just that vercel was the one that caught my eye

## Server Architecture Comparison

| Feature                | Dedicated Servers                                 | Cloudflare Pages        | Cloudflare Workers           | Fluid Compute (e.g., AWS wrapper) | AWS Lambda                                   |
| ---------------------- | ------------------------------------------------- | ----------------------- | ---------------------------- | --------------------------------- | -------------------------------------------- |
| Cost                   | High, fixed                                       | Low, usage-based        | Low, usage-based             | Potentially low, usage-based      | Low, usage-based                             |
| Scalability            | Limited, manual                                   | High, automatic         | High, automatic              | High, automatic, efficient        | High, automatic                              |
| Control                | Full                                              | Limited                 | Moderate, code-level         | Moderate, code-level              | Limited, code-level                          |
| Performance            | Excellent, consistent                             | Excellent for static    | Excellent, low latency       | Excellent, optimized              | Good, potential cold starts                  |
| Operational Complexity | High                                              | Low                     | Moderate                     | Moderate                          | Low                                          |
| Security               | High, user-managed                                | Mid, Cloudflare-managed | High, Cloudflare-managed     | High, provider-managed            | High, AWS-managed                            |
| Typical Scenarios      | High-traffic, enterprise, gaming, HPC, compliance | Static sites, JAMstack  | Edge logic, APIs, middleware | AI, real-time apps, streaming     | Event-driven, data processing, microservices |
